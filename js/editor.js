...
async function saveIssue(e) {
  ...
  try {
    // rockey_news에 브라우저가 직접 UPDATE/INSERT하지 않습니다.
    // RLS 때문에 저장이 막히는 문제를 방지하고, 관리자 권한을
    // SECURITY DEFINER RPC에서 다시 확인하도록 합니다.
    const oldNumber = state.isNew ? null : Number(state.row.news_number);
    const { error } = await sb.rpc("admin_save_news_issue", {
      p_old_news_number: oldNumber,
      p_news_number: payload.news_number,
      p_rockey_news: payload.rockey_news,
      p_question: payload.question,
      p_question_type: payload.question_type,
      p_choice1: payload.choice1,
      p_choice2: payload.choice2,
      p_choice3: payload.choice3,
      p_choice4: payload.choice4,
      p_choice5: payload.choice5,
    ...
    state.isNew=false;
    state.row=payload;
    history.replaceState(null,"",`./editor.html?issue=${encodeURIComponent(payload.news_number)}`);
    renderView();
  } catch(err){
    console.error(err);
    toast(err.message||"저장에 실패했습니다.",false);
    btn.disabled=false;
