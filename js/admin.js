...
async function verifyEmailToken(token){
  ...
  if(!value) throw new Error("인증 토큰이 없습니다.");
...
async function signIn(loginId,password){
  ...
  if(!id) throw new Error("계정 아이디를 입력하세요.");
  if(!pw) throw new Error("계정 비밀번호를 입력하세요.");

  clearVerified();

  $("#authStatus").textContent="관리자 계정을 확인하는 중…";
  const email=await resolveEmail(id);

  $("#authStatus").textContent="비밀번호를 확인하는 중…";
  const {data,error}=await sb.auth.signInWithPassword({
    email,
    password:pw
  });
  if(error) throw error;
  if(!data?.user) throw new Error("로그인 사용자를 확인할 수 없습니다.");

  $("#authStatus").textContent="관리자 권한을 확인하는 중…";
  await loadProfile(data.user);

  $("#authStatus").textContent="관리자 이메일 인증 메일을 보내는 중…";
  await requestEmailVerification();

  $("#authStatus").textContent =
    "로그인이 완료되었습니다. 관리자 이메일에서 인증 링크를 눌러 주세요.";
}
async function boot(){
  try{
