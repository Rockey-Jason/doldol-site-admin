...
async function box() {
  content.innerHTML = `
    <div class="news-center-head">
      ...
      <div class="news-head-actions">
        <button class="secondary" id="boxRefresh">↻ 새로고침</button><button class="primary" id="boxNew">＋ 아이템 생성</button>
      </div>
    </div>
  ...
  $("#boxNew").onclick = () => { window.location.href = "./box-editor.html?new=1"; };
...
/* =====================================================
   돌이 이슈
   이슈는 생성 시 돌돌증권의 6개 종목에 영향률을 적용합니다.
===================================================== */
...
async function issues() {
  content.innerHTML = `
    <div class="news-center-head">
      <div>
        <div class="eyebrow">DORI ISSUE / STOCK MARKET</div>
        <h1>📈 돌이 이슈</h1>
        <p>돌이 세계에서 발생한 주요 이슈를 등록하고, 6개 종목의 주가 변동을 함께 기록합니다.</p>
      </div>
      <div class="news-head-actions">
        <button class="secondary" id="issueRefresh">↻ 새로고침</button>
        <button class="primary" id="issueNew">＋ 이슈 생성</button>
      </div>
    </div>
    ...
    <div class="issue-overview">
      <div class="issue-overview-card">
        ...
        <b>이슈 기반 주가 반영</b>
        <small>생성 시 입력한 증감률이 현재가와 가격 이력에 원자적으로 반영됩니다.</small>
      </div>
      <div class="issue-overview-card">
        <span>6 STOCKS</span>
        <b>전 종목 필수 입력</b>
        <small>로키 페이퍼부터 돌이 의류까지 모든 종목의 영향률을 함께 기록합니다.</small>
      </div>
      <div class="issue-overview-card">
        <span>SAFE RANGE</span>
        <b>권장 범위 안내</b>
        <small>생성 화면에서 종목별 상황에 맞는 권장 증감 범위를 확인할 수 있습니다.</small>
      </div>
    </div>
    ...
    <div class="news-toolbar">
      <div class="search-box">
        <span>⌕</span>
        <input id="issueSearch" placeholder="이슈 번호, 제목, 내용 검색">
      </div>
    </div>

    <div class="panel news-panel">
      <div class="news-list-title">
        <div><h2>발행된 돌이 이슈</h2><span id="issueResultCount">불러오는 중…</span></div>
        <span class="sort-label">최신순 ↓</span>
      </div>
      <div id="issueList" class="special-main-list">불러오는 중...</div>
    </div>
  `;

  $("#issueRefresh").onclick = issues;
  $("#issueNew").onclick = () => { window.location.href = "./stock-issue-editor.html?new=1"; };
  $("#issueSearch").oninput = () => renderIssueList(window.__doriIssueRows || []);

  const { data, error } = await sb
    .from("dori_stock_issues")
    .select("id,issue_number,title,content,status,created_at,published_at")
    .order("issue_number", { ascending: false });

  if (error) {
    $("#issueList").innerHTML = `<div class="empty"><b>돌이 이슈를 불러오지 못했습니다.</b><br><small>${esc(error.message)}</small></div>`;
    return;
  }

  window.__doriIssueRows = data || [];
  renderIssueList(window.__doriIssueRows);
}

function renderIssueList(rows) {
  const q = String($("#issueSearch")?.value || "").trim().toLowerCase();
  const filtered = rows.filter(r =>
    [r.issue_number, r.title, r.content, r.status].join(" ").toLowerCase().includes(q)
  );

  $("#issueResultCount").textContent = `${filtered.length}개 표시`;

  if (!filtered.length) {
    $("#issueList").innerHTML = `
      <div class="issue-empty">
        <div>📈</div>
        <b>아직 발행된 돌이 이슈가 없습니다.</b>
        <small>오른쪽 위의 「이슈 생성」에서 첫 번째 이슈를 만들어 보세요.</small>
      </div>
    `;
    return;
  }

  $("#issueList").innerHTML = filtered.map((r, i) => `
    <div class="issue-list-row" style="--delay:${Math.min(i, 15) * 30}ms">
      <div class="issue-list-number"><span>ISSUE</span><b>#${esc(r.issue_number)}</b></div>
      <div class="issue-list-main">
        <b>${esc(r.title)}</b>
        <p>${esc(r.content || "내용 없음")}</p>
        <small>${esc(formatDate(r.published_at || r.created_at))}</small>
      </div>
      <span class="issue-published">● 발행됨</span>
    </div>
  `).join("");
}
...
async function events() {
  content.innerHTML = `
    <div class="news-center-head">
      ...
      <div class="news-head-actions">
        <button class="secondary" id="eventRefresh">↻ 새로고침</button><button class="primary" id="eventNew">＋ 이벤트 생성</button>
      </div>
    </div>
  ...
  $("#eventNew").onclick = () => { window.location.href = "./event-editor.html?new=1"; };
